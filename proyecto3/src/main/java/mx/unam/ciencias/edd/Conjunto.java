...
public class Conjunto<T> implements Coleccion<T> {
     ...
     * 
     * @return <code>true</code> si el conjunto es vacío, <code>false</code> en otro
     *         caso.
     */
    @Override
    ...
    public Conjunto<T> union(Conjunto<T> conjunto) {
        ...
        for (T elemento : conjunto.conjunto)
            union.agrega(elemento);
        return union;
    }
     ...
     * @return <code>true</code> si el objeto recibido es instancia de Conjunto, y
